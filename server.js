...
function saveShortcutData(data) {
  try { writeFileSync(DATA_FILE, JSON.stringify(data, null, 2)); } catch { console.error('Failed saving shortcut data'); }
}
...
app.get('/api/login', async (req, res) => {
  try {
    const testUser = { claims: { sub: 'test-user-1', email: 'stephaniebarchiesi@gmail.com', first_name: 'Stephanie', last_name: 'Barchiesi' } };
    await pool.query(`
    ...
    await new Promise((resolve, reject) => req.logIn(testUser, err => err ? reject(err) : resolve()));
...
app.get('/api/callback', async (req, res) => {
  try {
    ...
    if (!codeVerifier) return res.redirect('/api/login');
    ...
    const tokens = await oidcClient.authorizationCodeGrant(config, currentUrl, { pkceCodeVerifier: codeVerifier, expectedState: state });
    const claims = tokens.claims();
    const dbUser = await upsertUser(claims);

    const sessionUser = { claims, dbUser, access_token: tokens.access_token, refresh_token: tokens.refresh_token };
...
app.post('/api/data', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!updates || typeof updates !== 'object') return res.status(400).json({ message: 'Expected object' });
  ...
  try {
    if (token) {
      ...
      if (tokenResult.rows.length > 0) {
        ...
        console.log(`Shortcut DB saved for ${userId}`);
    ...
    console.log('Shortcut legacy saved', data[today]);
  ...
  try {
    ...
    if (req.isAuthenticated()) {
      try {
        ...
        const userData = {};
        dataResult.rows.forEach(r => userData[r.key]=r.value);
...
window.__GIORNO_USER__ = ${JSON.stringify({
  ...
  lastName: user.last_name, isPaid: user.is_paid||user.is_owner, isOwner: user.is_owner, shortcutToken
})};
window.__GIORNO_DATA__ = ${JSON.stringify(userData)};
      ...
      } catch(err) { console.error('HTML injection error:', err); }
    }
...
// ---- START SERVER ----
initDb().then(()=>app.listen(PORT,()=>console.log(`Giorno running on port ${PORT}`)))
.catch(err=>{ console.error('Failed to init DB:',err); process.exit(1); });

// ---- SHORTCUT DATA ENDPOINTS ----

// Get today's shortcut data (authenticated)
app.get('/shortcut-data', isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    console.error('Shortcut data DB error:', err);
  }
...
// Get today's shortcut data (unauthenticated, legacy only)
...
// Get last 30 days shortcut history (authenticated)
app.get('/shortcut-history', isAuthenticated, async (req, res) => {
  try {
    ...
    console.error('Shortcut history DB error:', err);
  }
